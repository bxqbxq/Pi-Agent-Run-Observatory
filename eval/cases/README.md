...
`add-validation-observed-recovery.json` 提供一组真实的失败与成功运行，用于验证 `/run-diff` 能否正确展示状态、验证结果、耗时和资源差异。失败由公开测试捕获，隐藏验收通过，说明问题来自 Agent 生成的实现与测试不一致，而不是 runner 启动或验收基础设施异常。
